...
class SlideshowBuilder:
    ...
    def build_ffmpeg_command(image_files: List[str], output_file: str, duration_per_image: int = 5, se_path: Optional[str] = None) -> (List[str], str, Optional[List[str]], Optional[str]):
        ...
        with open(list_path, 'w', encoding='utf-8') as f:
            # すべての画像にdurationとfileを指定し、最後の画像だけfileをもう一度書く
        ...
        if se_path:
            ...
            filter_complex = ";".join(adelay_filters) + ";" + "".join(amix_inputs) + f"amix=inputs={n}"
            audio_cmd = [
                'ffmpeg', '-y',
                *inputs,
    ...
    def run_slideshow(image_files: List[str], output_dir: str, log_func=print, duration_per_image: int = 5, se_path: Optional[str] = None, exif_enable: bool = False) -> Optional[str]:
        ...
        output_file = str(Path(output_dir) / f'slideshow_{dt_str}.mp4')
        tmp_dir = None
        ...
        try:
            ...
            if se_path and audio_cmd:
                ...
                if proc_a.returncode != 0:
                    ...
                    return None
                final_out = str(Path(output_file).with_suffix('.mux.mp4'))
                mux_cmd = [
                    'ffmpeg', '-y',
                    '-i', output_file,
                    '-i', audio_out,
                ...
                log_func(f'[完了] スライドショー動画生成成功: {output_file}')
                return output_file
        finally:
            ...
            if se_path and 'audio_out' in locals() and audio_out and Path(audio_out).exists():
